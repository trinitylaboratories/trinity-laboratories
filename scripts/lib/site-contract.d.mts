export const PRODUCTION_ORIGIN: 'https://trinitylaboratories.org';
export const PRODUCTION_HOST: 'trinitylaboratories.org';
export const CAPABILITY_SLUGS: readonly string[];
export const CAPABILITY_ROUTES: readonly string[];
export const PUBLICATION_IDS: readonly string[];
export const PUBLICATION_ROUTES: readonly string[];
export const CORE_ROUTES: readonly string[];
export const PORTAL_ROUTES: readonly string[];
export const FORM_IDS: readonly string[];
export const FORM_ROUTES: readonly string[];
export const SECURITY_RECORD_ROUTES: readonly string[];
export const RESEARCH_RECORD_ROUTES: readonly string[];
export const RECORD_ROUTES: readonly string[];
export const SEARCHABLE_FORM_ROUTES: readonly string[];
export const SEARCHABLE_RECORD_ROUTES: readonly string[];
export const RECORD_INDEX_ROUTES: readonly string[];
export const REPORT_IDS: readonly string[];
export const REPORT_ROUTES: readonly string[];
export const CONTROLLED_RECORD_ROUTES: readonly string[];
export const SITEMAP_ROUTES: readonly string[];
export const NOINDEX_ROUTES: readonly string[];
export const SITE_ROUTES: readonly string[];
export const LOCAL_FORM_ROUTES: readonly string[];
export const STUDY_ROUTES: readonly string[];
export const STUDY_FORM_ROUTES: readonly string[];
export const GATEWAY_ROUTE: '/employee-access/';
export const REQUIRED_GATEWAY_HOOKS: readonly string[];
export const REQUIRED_SESSION_HOOKS: readonly string[];
export function canonicalUrl(route: string): string;
export function normalizeRoute(route: string): string;
export function routeToOutputPath(route: string): string;
